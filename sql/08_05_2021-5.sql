...
CREATE TABLE [dbo].[student_subject] (
  ...
  CONSTRAINT [table_name_student_subject_to_StudentId] FOREIGN KEY
...
CREATE TABLE [dbo].[studentattendance_0] (
  ...
  CONSTRAINT [table_name_studentattendance_0_to_StudentId] FOREIGN KEY
   ([student_id]) REFERENCES [dbo].[Student] ([student_id]) ON DELETE CASCADE
);
...
BEGIN
DELETE FROM Teacher_Subject
  WHERE Teacher_Subject.Teacher_Id IN (SELECT
      Teacher_Id
    FROM deleted)

  DELETE FROM Teacher
  WHERE Teacher.Teacher_Id IN (SELECT
      Teacher_Id
    FROM deleted)
...
CREATE TRIGGER Teacher_Subject_delete
ON Teacher_Subject
INSTEAD OF DELETE
AS
BEGIN
DELETE FROM Attendance
  WHERE Attendance.Teacher_Subject_Id IN (SELECT
      Teacher_Subject_Id
    FROM deleted)

DELETE FROM OTP
  WHERE OTP.Teacher_Subject_Id IN (SELECT
      Teacher_Subject_Id
    FROM deleted)

DELETE FROM Teacher_Subject
  WHERE Teacher_Subject.Teacher_Subject_Id IN (SELECT
      Teacher_Subject_Id
    FROM deleted)
END
GO
